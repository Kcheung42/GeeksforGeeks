...
#    Updated: 2018/03/23 10:06:31 by kcheung          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
class Graph:
	...
	def add_edge(self, u, v, weight):
		if u in self.vertices and v in self.vertices:
			for key, val in self.vertices.items():
				if key == u:
					val.add_neighbor((weight,self.vertices[v]))
				if key == v:
					val.add_neighbor((weight,self.vertices[u]))
		else:
	...
	def dijkstra(self, f, t):
		...
		while q:
			...
			if v1 not in seen:
				seen.add(v1)
				path = (v1, path) #continuously append path
			if v1 == t: return (cost, path)
			for c, vertex in self.vertices[v1].neighbors:
				v2 = vertex.name
				if v2 not in seen:
